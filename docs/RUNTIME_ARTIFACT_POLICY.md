...
- `oma-dashboards/` generated by `open-multi-agent` CLI runs
- `runtime_invocation_report.json` produced inside an Artifact Harness packet run as dry-run guard evidence
- `approval_evidence.json` produced inside an Artifact Harness packet run as explicit gate-decision evidence
- `repair_plan.json` produced inside an Artifact Harness packet run as advisory recovery evidence
