...
- `Roster` is the stable user-facing Artifact Harness workflow alias. After `roster-install` and a Codex reload, `@roster` and `/roster` are the intended installed plugin/slash invocation targets. `PM` is accepted only as an unambiguous artifact-task alias.
